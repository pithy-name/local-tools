...
- **Credentials** (e.g. `.credentials.json`) — Cowork auth tokens at the workspace root. The script's discovery globs (`local_*.json` sidecars and `*.jsonl` transcripts under session dirs) never reach these by design. As defense-in-depth, invariant **I4** flags any newly-added file whose name contains `credentials` (case-insensitive), not just `.credentials.json`.
